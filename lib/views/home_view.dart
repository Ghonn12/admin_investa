...
class HomeView extends StatelessWidget {
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                Obx(() => Column(
                  children: [
                    _buildSidebarItem(Icons.dashboard, "Dashboard", 0, isActive: controller.activeTab.value == 0),
                    _buildSidebarItem(Icons.people, "User Management", 1, isActive: controller.activeTab.value == 1),
                    _buildSidebarItem(Icons.receipt_long, "Transactions", 2, isActive: controller.activeTab.value == 2),
                  ],
                )),
                const Spacer(),
                _buildSidebarItem(Icons.logout, "Logout", 99, isActive: false, onTap: controller.logout),
                const SizedBox(height: 24),
              ],
            ),
          ),
  ...
  Widget _buildTransactionContent() {
    return Column(
      ...
      children: [
        const Text("Log Aktivitas Transaksi", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
        const SizedBox(height: 24),
        Expanded(
          child: Container(
                ...
                return SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    columns: const [
                      ...
                      DataColumn(label: Text('Judul/Kategori', style: TextStyle(fontWeight: FontWeight.bold))),
                    ...
                    rows: controller.transactions.map((trx) {
                      ...
                      return DataRow(cells: [
                        ...
                        DataCell(Column(
                          ...
                          children: [
                            Text(trx['user_name'] ?? 'N/A', style: const TextStyle(fontWeight: FontWeight.w600)),
                            Text(trx['user_email'] ?? '', style: const TextStyle(fontSize: 12, color: AdminColors.textGrey)),
                          ],
                        )),
                        DataCell(
                          Container(
                        ...
                        DataCell(Text(trx['title'] ?? trx['category'] ?? '-')),
                        DataCell(
                            Text(
  ...
  Widget _buildTopBar(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Obx(() => Text(
              controller.activeTab.value == 0 ? "Dashboard Overview" :
              controller.activeTab.value == 1 ? "User Management" : "Transaction Log",
              style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)
          )),
          const Row(
            children: [
              CircleAvatar(backgroundColor: AdminColors.bg, child: Icon(Icons.person, color: AdminColors.text)),
              SizedBox(width: 12),
              Column(
                mainAxisAlignment: MainAxisAlignment.center,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("Admin", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                  Text("Super Admin", style: TextStyle(color: AdminColors.textGrey, fontSize: 12)),
                ],
              )
            ],
          )
  ...
  Widget _buildSidebarItem(IconData icon, String title, int index, {bool isActive = false, VoidCallback? onTap}) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: isActive ? const Border(left: BorderSide(color: AdminColors.secondary, width: 4)) : null,
        ),
        child: Row(
          children: [
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _buildStatCard("Total Users", "${controller.stats['total_users'] ?? 0}", Icons.people, Colors.blue),
              const SizedBox(width: 24),
              _buildStatCard("Total Transactions", "${controller.stats['total_transactions'] ?? 0}", Icons.receipt, Colors.orange),
              const SizedBox(width: 24),
              _buildStatCard("Total Circulation", "Rp ${controller.stats['total_balance'] ?? 0}", Icons.monetization_on, Colors.green),
            ],
          ),
              ...
              children: [
                ...
                Expanded(
                  child: Obx(() {
                    ...
                    return LineChart(
                      _mainData(controller.userGrowthData.toList(), controller.labels.toList()),
            ...
            getTitlesWidget: (value, meta) {
              ...
              if (index >= 0 && index < labels.length) {
                return SideTitleWidget(
                  ...
                  child: Text(labels[index], style: const TextStyle(fontSize: 12, color: AdminColors.textGrey)),
                );
              }
        ...
        LineChartBarData(
          ...
          belowBarData: BarAreaData(
            ...
            gradient: LinearGradient(
              colors: [AdminColors.primary.withOpacity(0.4), AdminColors.primary.withOpacity(0.0)],
                          ...
                          children: [
                            ...
                            Column(
                              ...
                              children: [
                                ...
                                Text(user['email'], style: const TextStyle(fontSize: 12, color: AdminColors.textGrey)),
                              ],
                            )
                          ],
                        )),

                        DataCell(Text("Rp ${user['balance']}", style: const TextStyle(fontFamily: 'monospace'))),
                        ...
                        DataCell(Row(
                          children: [
                            ...
                            IconButton(
                              ...
                              tooltip: "Hapus User",
                              onPressed: () => controller.deleteUser(int.parse(user['id'].toString())),
                            ),
                          ],
                        )),
                      ]);
        ...
        child: Row(
          children: [
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(title, style: const TextStyle(color: AdminColors.textGrey, fontSize: 14)),
